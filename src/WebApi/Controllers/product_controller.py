from fastapi import FastAPI, HTTPException, Path, Body, APIRouter, JSONResponse
from pydantic import BaseModel
from bson import ObjectId
from Application.Contracts.iproduct_service import ProductServiceInterface  # Import the ProductServiceInterface
from Application.UseCases.Services.product_service import ProductService  # Import the ProductService
...
from WebApi.Controllers.user_controller import app
# Initialize the product service using the interface
product_service: ProductServiceInterface = ProductService()  # Replace with your actual implementation
...
def create_product(product_data: ProductCreate):
    ...
    return JSONResponse(content={'message': 'Product created', 'product_id': str(product_id)})
...
def update_product(product_id: ObjectId = Path(..., title="The product ID"), product_data: ProductCreate = Body(..., title="Updated product data")):
    product = CreateProductDto(**product_data.dict())
